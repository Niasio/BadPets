...
public class SettingsManager {
    ...
    private SettingsManager(String fileName) {
        ...
        if (!Pets.getPlugin().getDataFolder().exists()) {
            Pets.getPlugin().getDataFolder().mkdir();
        }

        File f = new File(Pets.getPlugin().getDataFolder(), "/data");
        if (!f.exists())
            f.mkdirs();

        file = new File(Pets.getPlugin().getDataFolder(), fileName + ".yml");
