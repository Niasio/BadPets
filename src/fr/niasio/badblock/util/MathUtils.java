...
public class MathUtils {
    ...
    public static void applyVelocity(final Entity ent, Vector v) {
        ...
        if(ent instanceof Player) {
            CustomPlayer customPlayer = Pets.getCustomPlayer((Player)ent);
            if(!customPlayer.hasGadgetsEnabled())
                return;
        }
        ent.setVelocity(v);
        Bukkit.getScheduler().runTaskLater(Pets.getPlugin(), new Runnable() {
            @Override
            public void run() {
                Pets.noFallDamageEntities.add(ent);
            }
        }, 4);
    }
