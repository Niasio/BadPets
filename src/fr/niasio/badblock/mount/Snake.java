...
import fr.niasio.badblock.util.MathUtils;
...
public class Snake extends Mount {

    private HashMap<Player, ArrayList<Entity>> tailMap = new HashMap<Player, ArrayList<Entity>>();
    private int color = 1;


    public Snake(UUID owner) {
        super(EntityType.SHEEP, Material.SEEDS, (byte) 0, "Snake", "pets.snake", owner, MountType.SNAKE);
        ...
        tailMap.put(getPlayer(), new ArrayList<Entity>());
        ((ArrayList<Entity>) tailMap.get(getPlayer())).add(ent);
        addSheepToTail(4);
